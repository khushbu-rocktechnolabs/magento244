                ...
                if (array_key_exists('conditions', $cond)) {
                    foreach ($cond['conditions'] as $rulecond) {
                        if ($rulecond['attribute'] == 'total_qty') {
                            $valid = false;
                            $code = 'if((int)$cart->getQuote()->getItemsQty()'.$rulecond['operator'].' (int)$rulecond["value"]){$trueskus[] = $ruleCollection->getFreeGiftSku(); $validArray[] = true; }else{ $falsesku[] = $ruleCollection->getFreeGiftSku(); $validArray[] = false; $valid = true; }';
                            eval($code);
                        }
